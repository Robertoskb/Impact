...
function testSkillsReport() {
  ...
  testData.questions.forEach((question) => {
    ...
    if (question.cancelled) {
      ...
      const correctAnswer =
        testData.meta[2023][area][question.position]?.answer;
  ...
  Object.keys(skillsData).forEach((area) => {
    Object.keys(skillsData[area]).forEach((hability) => {
      ...
      console.log(
        `   H${hability}: ${skillData.correct}/${answeredQuestions} = ${percentage}% (${skillData.cancelled} anuladas)`
      );
    });
  });
...
function testResultsPattern() {
  ...
  testData.questions.forEach((question) => {
    ...
    if (!isCancelled) {
      const correctAnswer =
        testData.meta[2023][question.area][question.position]?.answer;
      const userAnswer = testData.answers[question.position];
...
function testGeneralStats() {
  ...
  console.log(
    `   Performance: ${performance}% (${correctAnswers}/${validQuestions})`
  );
