...
console.log(
  `Usuário fez prova ${testScenario.config.color.toUpperCase()} de ${
    testScenario.config.year
  }`
);
    ...
    if (yearData && yearData[question.area]) {
      for (const [originalPos, colorMap] of Object.entries(
        ...
        if (colorMap[mappedColor] === question.position) {
          ...
          console.log(
            `✅ Questão ${question.position} (rosa) → Posição ${realPosition} (azul)`
          );
          break;
        }
      }
    ...
    if (yearData && yearData[question.area]) {
      for (const [originalPos, colorMap] of Object.entries(
        ...
        if (colorMap[mappedColor] === question.position) {
          ...
          const metaData =
            testScenario.meta[testScenario.config.year][question.area][
              mappedPosition
            ];
          ...
          if (metaData && metaData.answer) {
            ...
            console.log(
              `📝 Questão ${
                question.position
              }: Usuário=${userAnswer}, Gabarito=${metaData.answer} → ${
...
function testSkillsReport() {
      ...
      for (const [originalPos, colorMap] of Object.entries(
        ...
        if (colorMap[mappedColor] === question.position) {
          ...
          if (metaData && metaData.hability) {
            ...
            console.log(
              `📚 H${hability}: Questão ${question.position} → ${
                userAnswer === correctAnswer ? "ACERTO" : "ERRO"
              }`
            );
          }
          break;
  ...
  Object.keys(skillsData).forEach((area) => {
    Object.keys(skillsData[area]).forEach((hability) => {
      ...
      console.log(
        `   H${hability}: ${skill.correct}/${skill.total} = ${percentage}%`
      );
    });
  });
}
...
console.log(
  "3. SkillsReportCalculator - usa posição mapeada para buscar habilidade"
);
