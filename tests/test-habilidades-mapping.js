...
console.log(`Usuário escolheu: Ano ${userConfig.year}, Cor ${userConfig.color}`);
console.log('Questões que o usuário vê (posições na cor amarela):');
userQuestions.forEach(q => console.log(`- Questão ${q.position} (${q.area})`));
    ...
    if (yearData && yearData[question.area]) {
      ...
      for (const [originalPos, colorMap] of Object.entries(yearData[question.area])) {
        if (colorMap[mappedColor] === question.position) {
          ...
          const metaData = mockMeta[userConfig.year][question.area][realPosition];
          
          console.log(`📍 Questão ${question.position} (${userConfig.color}) → Posição ${realPosition} (azul)`);
          if (metaData) {
