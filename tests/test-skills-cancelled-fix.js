...
console.log(
  "🧪 TESTE: Verificando tratamento de questões anuladas no SkillsReportCalculator\n"
);
...
function testSkillsReportCalculator() {
  ...
  testData.questions.forEach((question) => {
    ...
    if (question.cancelled) {
      console.log(
        `⏭️ Questão ${question.position} (${area}) pulada - questão anulada (sem habilidade conhecida)`
      );
      return;
    }
    ...
    if (
      !testData.meta[2022][area] ||
      !testData.meta[2022][area][question.position]
    ) {
      console.warn(
        `Metadados não encontrados para questão ${question.position} (${area})`
      );
      return;
    }
    ...
    if (!hability) {
      console.warn(
        `Habilidade não encontrada para questão ${question.position} (${area})`
      );
      return;
    }
  ...
  Object.keys(skillsData).forEach((area) => {
    Object.keys(skillsData[area]).forEach((hability) => {
      ...
      console.log(
        `   H${hability}: ${skillData.correct}/${skillData.total} = ${percentage}%`
      );
      console.log(
        `     Questões: [${skillData.questions
          .map((q) => q.position)
          .join(", ")}]`
      );
    });
  ...
  console.log(
    "✅ Não houve tentativa de buscar habilidade para questões anuladas"
  );
...
console.log("\n🎉 RESUMO DA CORREÇÃO:");
console.log(
  "- Questões anuladas são filtradas ANTES de tentar buscar habilidades"
);
console.log(
  "- Evita erro de tentar buscar habilidade de questões que não estão no meta.json"
);
